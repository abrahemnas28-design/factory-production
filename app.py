...
# הגדרות קבצים
DATA_FILE = "production_summary_v2.csv"

# הגדרת המחלקות
DEPARTMENTS = {
    "מחלקת פלסטיק": ["מכונה סנו", "מכונה פרסיל2.5", "מכונה פרסיל4", "מכונה סוד", "מכונה סינית"],
...
# פונקציה לטעינת נתונים בטוחה
def load_data():
    if not os.path.exists(DATA_FILE):
        return pd.DataFrame(columns=["date", "dept", "machine", "count"])
    try:
...
st.markdown("""
    ...
    .machine-label { font-size: 24px; font-weight: bold; padding-top: 20px; text-align: right; }
...
st.sidebar.download_button("📥 הורד דוח ייצור", data=csv_data, file_name=f"report_{today}.csv")
